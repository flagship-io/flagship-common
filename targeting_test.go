...
func TestComplexTargeting(t *testing.T) {
	...
	context.Rows["accountName"] = structpb.NewStringValue("Flagship Demo")
	context.Rows["featureType"] = structpb.NewStringValue("deployment")
	test, err := targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.True(t, test)

	context.Rows["featureType"] = structpb.NewStringValue("ab")
	test, err = targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.False(t, test)

	context.Rows["gender"] = structpb.NewStringValue("female")
	test, err = targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.False(t, test)

	context.Rows["isVIP"] = structpb.NewBoolValue(true)
	test, err = targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.True(t, test)
	...
	context = targeting.Context{
		Rows: targeting.ContextRows{},
		Providers: map[string]targeting.ContextRows{
			"myprovider": {},
		},
	}
	context.Providers["myprovider"] = targeting.ContextRows{
	...
	context.Providers["myprovider"]["gender"] = structpb.NewStringValue("male")
	context.Providers["myprovider"]["country"] = structpb.NewStringValue("EN")
	test, err = targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.False(t, test)

	context.Providers["myprovider"]["country"] = structpb.NewStringValue("FR")
	test, err = targetingMatch(targetingsTest, "test@abtasty.com", context)
	assert.Nil(t, err)
	assert.True(t, test)
}
